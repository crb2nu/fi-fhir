...
- **U-0 (MR !233)** — design tokens rewritten dark-first with the legacy token names kept, a `[data-theme="light"]` set, and primitives in `ui/src/lib/ui/primitives/` (Button, IconButton, Tabs, Badge, Panel, Toolbar, Table/Th/Td/Tr, Field/Input/Select/Textarea, EmptyState, KeyValue, Popover, Icon) on `@lucide/svelte` 1.48.0; a dev-only `/design` gallery and `ui/docs/DESIGN.md`.
- **U-1 (MR !235)** — the shell: a 40 px header with the five-stage `StageControl` (one tab stop, arrow keys), a breadcrumb, Commands and the theme toggle; the status bar carries connection state, the access chip (Trusted network, Cloudflare Access · name, or Bearer, with "Clear access" for bearer sessions only) and "Next"; the signed-in credential strip is gone (the sign-in is still announced to assistive tech) and the bearer entry is a centred dialog; `JourneyProgress`, `DocumentHost` and the editor-less document types are deleted and saved layouts drop them; Home and Operator are named the same in every place; a layout saved on Operator restores (`operator` was missing from the valid views); the document region pads 12 px unless the route renders a `Toolbar`.
- **U-2 (MR !236)** — Home is Recent (open documents and the tenant's integration sessions), Integrations (deployments, behind the same operator pre-flight as `/operator`), Health (backend, GraphQL and the shell poll as table rows) and Alerts; `observabilityStore` has no generators, and `alertSource` (`unconfigured`, `live`, `unavailable`) says why a list is empty; Events and Operator are toolbars over tables with details panes; a slow message-trace answer can no longer overwrite a newer selection; `StreamingUnavailable` keeps its gate contract under a new contract test.
- **U-3 (MR !234)** — HL7 intake is editor-first (the editor takes most of the page) with pipeline chips that show only what a run used, one status line and dense results tables; workflows, profiles and terminology are toolbars over tables with details panes; `AuthoringFlowRail` and `LifecycleTrace` are deleted. Fixed on the way: several reactivity bugs (workflow readiness and publish enablement, terminology and sample bulk selection), a terminology upload that never refreshed the list, Inspector field clicks that did nothing, a failed preview that showed no error while the session engine was on, and editor text overwritten by unrelated sample-list updates.
- **U-4 (MR !237)** — `test:ui-e2e` gains a `visual` project that captures 1440×900 PNGs of every route, the bottom panel and the command palette as job artifacts, and fails if any retired marketing phrase reappears; no golden images are committed.
- **Production** — UI `v0.1.29328` verified from the LAN at 1440×900 on every route, including an HL7 Preview on the session engine.
...
- **R-E (MR !229, from R-D's finding)** — HL7 intake sends CR-terminated segments on every path (stateless preview, Integration Session run, process/submit) whatever the editor holds: CodeMirror joins lines with LF and pasted files carry LF or CRLF, and the preview kernel's strict validation accepts only CR. The editor text is left as typed; the manual "Normalize newlines" action stays. The built-in sample was never executable — literal `\r` characters for separators, a doubled backslash in the MSH-2 encoding characters, and no EVN segment — and is now a CR-terminated four-segment ADT^A01. `internal/parser/hl7v2` already split segments tolerantly; a table test now pins identical parses across CR, LF and CRLF so that stays true. The strict line-ending gate is deliberately unchanged.
