...
const (
	...
	// DocumentReference code systems
	SystemDocumentReferenceStatus   = "http://hl7.org/fhir/document-reference-status"
	SystemCompositionStatus         = "http://hl7.org/fhir/composition-status"
	SystemDocumentReferenceCategory = "http://hl7.org/fhir/us/core/CodeSystem/us-core-documentreference-category"
	SystemDocumentType              = "http://loinc.org" // LOINC for document types
	SystemDocumentClass             = "http://loinc.org" // LOINC for document class
	SystemDocumentFormat            = "http://ihe.net/fhir/ValueSet/IHE.FormatCode.codesystem"
	SystemConfidentiality           = "http://terminology.hl7.org/CodeSystem/v3-Confidentiality"
	SystemDocumentRelation          = "http://hl7.org/fhir/document-relationship-type"
	SystemFacilityType              = "http://snomed.info/sct" // SNOMED CT for facility types
	SystemPracticeSetting           = "http://snomed.info/sct" // SNOMED CT for practice settings

	// DiagnosticReport code systems
	SystemDiagnosticReportStatus   = "http://hl7.org/fhir/diagnostic-report-status"
	SystemDiagnosticReportCategory = "http://loinc.org" // LOINC for report category
...
// Attachment represents document/media content.
type Attachment struct {
	ContentType string `json:"contentType,omitempty"` // MIME type
	Language    string `json:"language,omitempty"`    // Human language
	Data        string `json:"data,omitempty"`        // Base64-encoded data
	URL         string `json:"url,omitempty"`         // URI where data can be found
	Size        int64  `json:"size,omitempty"`        // Number of bytes
	Hash        string `json:"hash,omitempty"`        // SHA-1 hash (base64)
	Title       string `json:"title,omitempty"`       // Label to display
	Creation    string `json:"creation,omitempty"`    // Date attachment was created
}
...
// Conforms to US Core DocumentReference profile.
type DocumentReference struct {
	ID            string                       `json:"id,omitempty"`
	Meta          *Meta                        `json:"meta,omitempty"`
	Identifier    []Identifier                 `json:"identifier,omitempty"`
	Status        string                       `json:"status"`                  // current | superseded | entered-in-error
	DocStatus     string                       `json:"docStatus,omitempty"`     // preliminary | final | amended | entered-in-error (composition status)
	Type          *CodeableConcept             `json:"type"`                    // Kind of document (LOINC required by US Core)
	Category      []CodeableConcept            `json:"category"`                // Categorization of document (US Core requires)
	Subject       *Reference                   `json:"subject"`                 // Who/what is the subject (US Core requires Patient)
	Date          string                       `json:"date,omitempty"`          // When document was indexed
	Author        []Reference                  `json:"author,omitempty"`        // Who and/or what authored the document
	Authenticator *Reference                   `json:"authenticator,omitempty"` // Who/what authenticated the document
	Custodian     *Reference                   `json:"custodian,omitempty"`     // Organization maintaining the document
...
// DocumentReferenceContent represents the document content.
type DocumentReferenceContent struct {
	Attachment *Attachment `json:"attachment"`       // Where to access the document
	Format     *Coding     `json:"format,omitempty"` // Format/content rules for the document
}

// DocumentReferenceContext represents the clinical context of the document.
type DocumentReferenceContext struct {
	Encounter         []Reference       `json:"encounter,omitempty"`         // Context of the document
	Event             []CodeableConcept `json:"event,omitempty"`             // Main clinical acts documented
	Period            *Period           `json:"period,omitempty"`            // Time of service documented
	FacilityType      *CodeableConcept  `json:"facilityType,omitempty"`      // Kind of facility
	PracticeSetting   *CodeableConcept  `json:"practiceSetting,omitempty"`   // Additional details about where
	SourcePatientInfo *Reference        `json:"sourcePatientInfo,omitempty"` // Patient demographics from source
	Related           []Reference       `json:"related,omitempty"`           // Related identifiers or resources
}

// GetResourceType returns "DocumentReference".
func (dr *DocumentReference) GetResourceType() string {
...
// ============================================================================
// DiagnosticReport for Clinical Notes (US Core)
// ============================================================================

// DiagnosticReportNote represents a clinical note/report (not lab).
// This is the DiagnosticReport for Report and Note Exchange profile.
// Note: We reuse the base DiagnosticReport structure but with note-specific mappings.
// The existing DiagnosticReport type can be used for clinical notes as well,
// but this alias provides clarity for the specific use case.
type DiagnosticReportNote struct {
	...
	Status             string                  `json:"status"`                       // registered | partial | preliminary | final | amended | corrected | appended | cancelled | entered-in-error | unknown
	Category           []CodeableConcept       `json:"category"`                     // Service category (US Core requires)
	Code               *CodeableConcept        `json:"code"`                         // Name/code for report (US Core requires)
	Subject            *Reference              `json:"subject"`                      // Patient reference (US Core requires)
	Encounter          *Reference              `json:"encounter,omitempty"`          // Health care event when test ordered
	EffectiveDateTime  string                  `json:"effectiveDateTime,omitempty"`  // Clinically relevant time (US Core requires effective[x])
	EffectivePeriod    *Period                 `json:"effectivePeriod,omitempty"`    // Clinically relevant time (period)
	...
	ImagingStudy       []Reference             `json:"imagingStudy,omitempty"`       // Reference to imaging studies
	Media              []DiagnosticReportMedia `json:"media,omitempty"`              // Key images associated with report
	Conclusion         string                  `json:"conclusion,omitempty"`         // Clinical conclusion
	ConclusionCode     []CodeableConcept       `json:"conclusionCode,omitempty"`     // Codes for the clinical conclusion
	PresentedForm      []Attachment            `json:"presentedForm,omitempty"`      // Entire report as issued (US Core requires for notes)
}

// DiagnosticReportMedia represents key images associated with report.
type DiagnosticReportMedia struct {
	Comment string     `json:"comment,omitempty"` // Comment about the image
	Link    *Reference `json:"link"`              // Reference to the image source
}

// GetResourceType returns "DiagnosticReport".
func (drn *DiagnosticReportNote) GetResourceType() string {
	return "DiagnosticReport"
}

// MarshalJSON ensures ResourceType is always set.
func (drn *DiagnosticReportNote) MarshalJSON() ([]byte, error) {
	type Alias DiagnosticReportNote
	return json.Marshal(&struct {
		ResourceType string `json:"resourceType"`
		*Alias
	}{
