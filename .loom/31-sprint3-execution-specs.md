...
| `internal/integration/processor/migrations/` | **C** | 4.2a took `0003_operator_control_plane.sql`. C takes `0004_*`. Note the number in a `.loom/worklog/` entry, but treat it as a note and not a reservation — see the session row below, and re-check the directory against `origin/main` immediately before you commit. |
| `internal/integration/session/migrations/` | **C, then A** | **Corrected by S3-A, 2026-08-08.** As written this row assigned session `0004_*` to C alone, without noticing that Lane S3-A task 6 (durable session fanout) also needs a session migration. Both lanes need one. **Settled by merge order, not by claim: S3-C merged first and took `0004_export_attribution.sql`, so S3-A renumbered to `0005_session_stream_events.sql`.** The lesson for the next sprint is that a migration number cannot be reserved in a worklog while another lane can merge ahead of you — the ledger, not the document, is the authority. |
...
- Before the first commit, each lane records its owned files and its claimed migration number in its own `.loom/worklog/` entry (`make worklog-new TITLE="Lane X file ownership"`). One file per entry, so lanes never conflict on the worklog itself.
