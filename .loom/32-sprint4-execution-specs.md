...
**24. The repository does not define "one version".** There are **zero** git tags. `const version = "0.1.0"` is hardcoded and never bumped (`cmd/fi-fhir/main.go:57`), and S3-A's `fi_fhir_build_info` metric carries it, so every replica in a rolling upgrade reports the same version.

> **Corrected by Lane S4-C, 2026-08-09 (partially wrong, and the correction strengthens the conclusion).** CI-built binaries *do* carry a changing version: `test:binary` (`.gitlab-ci.yml:498`), `build:binary` (`:1835`), and the release matrix (`:2259-2279`) all pass `-ldflags "-X main.version=${CI_COMMIT_TAG:-${CI_COMMIT_SHORT_SHA}}"`, and `make build-release` uses `git describe`. What is true is narrower and worse: `make build` passes no ldflags, so a developer build reports the hardcoded default — and, decisively, **the binary version is not the compatibility boundary in either case**. Two binaries built from different commits may expect identical schemas; two reporting the same string may not. So N-1 cannot be defined on the version string even after it starts changing. The lane's fix is therefore not "bump a constant" but "report the six ledger versions", plus changing the unstamped default from the release-looking `0.1.0` to `0.0.0-dev`. See `.loom/40-decisions.md` (2026-08-09). Six independent forward-only migration ledgers exist — `integration_submission_schema_migrations`, `integration_session_schema_migrations`, `integration_lifecycle_schema_migrations`, `integration_batch_schema_migrations`, `integration_destination_schema_migrations`, and `terminology.schema_version` (`postgres_submission.go:134`, `session/postgres.go:76`, `lifecycle/postgres.go:65`, `batch/store.go:112`, `destination/postgres.go:57`, `pkg/terminology/db/schema.go:7,515-522`) — with no down path in any of them. "N-1" must be **defined** before it can be proved.
...
| `internal/integration/processor/migrations/0005_*`, ~~`internal/integration/session/migrations/0006_*`~~ **`0007_*`** | **B only** | Re-verify the numbers against `origin/main`'s `migrations/` directories on **every** rebase; the ledger is the authority (correction 40). **Corrected 2026-08-09:** Lane S4-C took session `0006_export_attribution_defaults.sql`, because task 2's rollback fix is a session schema change and this map did not anticipate that its own task list required one. S4-C merges first, so **S4-B's session migration is `0007`**. S4-C also appends one embed and one ledger step to `internal/integration/session/postgres.go`, which this map assigns to B — an append-only edit in the same shape B will make, in a file B rebases onto after S4-C merges. |
...
| `TestMigrationCompatibility_ExportInsertShapeSurvivesOneVersionRollback` | S4-C | **Fail**, on a not-null violation on `principal_json` | "rollback already works" — **CONFIRMED 2026-08-09 (MR !151)**: `SQLSTATE 23502 (not_null_violation) on column "principal_json" of relation "integration_session_exports"`, both locally and in CI. Correction 23 stands. |
