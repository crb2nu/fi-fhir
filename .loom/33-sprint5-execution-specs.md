...
**8. `fi-fhir workflow loadtest` is worse than "measures the legacy engine".** `cmd/fi-fhir/main.go:1955` → `runWorkflowLoadtest` (`:2758`) builds `workflow.NewEngine(wf)` (`:2868`) and `workflow.NewLoadTester(engine)` (`:2916`); `internal/workflow/loadtest.go` imports only stdlib (`:3-11`). **Executed** against the repo's own `configs/adt-workflow.yaml`: `Events/sec: 499.98`, `Achievement: 100.0% of target`, `P99: 103.792µs` — and `Errors: Count: 1000, Rate: 100.0000%`, with 1499 × `template: msg:1: function "event" not defined` on stderr. Two shipped-artifact defects: `configs/adt-workflow.yaml:14` uses templating the engine does not implement, and ~~`NewHealthcareEventGenerator` (`loadtest.go:586-626`) emits events that fall through every route to the catch-all~~ — **corrected by S5-A, 2026-08-09: the generator is not the fault, and the real cause is worse.** `NewHealthcareEventGenerator` (`loadtest.go:586-626`) emits exactly the four `patient_*` types the named routes match (weights 25/15/10/5) plus three types (`lab_result`, `appointment_scheduled`, `claim_submitted`, 45% of weight) that match no route at all. The 100% error rate comes from the `emit` action, and **`emit` is not a registered action type at all** — `engine.go:125-134` registers `log`, `webhook`, `fhir`, `email`, `exec`, `file`, `database`, `queue`, `event_store`, `athena`, and nothing else. Every action in the file failed with `unknown action type: emit`, on every event, under `fi-fhir workflow run` as well as under the loadtest. Proved by substitution — replacing `type: emit` with `type: log` in a copy of `configs/adt-workflow.yaml`, changing nothing else, drops the error rate from 100.0000% to 0.0000% at the same event rate.

**And `configs/adt-workflow.yaml` carries a third defect, worse than the other two and named by nobody: its routing never worked.** `condition:` is a field of `Filter` (`internal/workflow/types.go:19-31`), so it belongs under `filter:`; at the route's top level it is an unrecognized key, silently dropped, leaving a zero-value `Filter` — **which matches every event**. Measured on 4 events against the shipped file: **20 route matches**, i.e. all five routes fired on all four events, including a `lab_result` event the file has no route for. After repair: 3 route matches, 0 errors. The three defects partly masked each other, and the one signal that would have exposed them — the route-match count — is asserted nowhere in the repository. This also raises the stakes past "a stale sample": `docker-compose.yaml:80` mounts this file as `FI_FHIR_WORKFLOW_CONFIG_PATH` and `docs/user-guide/getting-started.md:253` passes it to `fi-fhir serve --workflow`, so match-everything routing was live in the default development stack.

A log-only probe confirms the corrected template form renders (`{{.type}}`, `{{.patient.mrn}}` → `ok patient_admit mrn=12345`), so the templating defect is `configs/adt-workflow.yaml` using an `event.`-prefixed, `.data.`-nested form the engine never implemented; commit `5d07101c4` fixed exactly this across `examples/workflows/*.yaml` and the user guide, and missed `configs/`, the one directory the loadtest usage text points at. **The correction strengthens the conclusion rather than weakening it**: the headline `Achievement: 100.1% of target` is not merely measuring the wrong engine, it is structurally blind to whether any action succeeded — it is an event *emission* rate, and it reads 100% whether the workflow works perfectly or fails on every single event. Its pass constants are hardcoded at `main.go:2931` — `result.Passed(0.90, 0.01, 10*time.Millisecond)` — unrelated to the spec's p95 ≤250 ms / p99 ≤500 ms. Reusing this harness would time the legacy engine *failing*. Confirms and strengthens `.loom/32` correction 22.
...
**19. `test/e2e/integration_test.go` has five tests and seven `t.Skipf` calls; one test needs no infra.** — **CORRECTED 2026-08-09 by Lane S5-B, by running it.** The framing below is right about the mechanics and wrong about the consequence: it reads as though the tests are correct and merely unrun, so task 5 would be "remove or repair the skips". Executed against real services at `2f8b3f609`, the tree is **red**. `test/e2e/e2e_test.go` — build tag `e2e` alone, nine tests, no external infrastructure, the file `make test-e2e` runs — fails four ways: `TestWorkflowCELFilter` (3 subtests), `TestWorkflowTransform`, `TestConfigValidation/invalid_cel`, and `TestEndToEndPipeline` with a nil-interface **panic** at `e2e_test.go:493`. In `integration_test.go`, `TestDatabaseAction` and `TestWebhookAction` fail with `workflow run failed: exit status 1` with PostgreSQL and the echo destination both reachable, and `TestWorkflowWithRetry` — the one needing no infrastructure — fails its own timing assertion. Only `TestObservabilityEndpoints` passes. One cause covers most of it: **workflow-schema drift.** These tests write Go dot-path templates (`"Status: {{.Patient.Status}}"`, `e2e_test.go:366`) while the engine binds JSON snake_case keys — the drift commit `5d07101c4` rewrote every document to correct, three commits before this sprint, leaving the tests behind. So `test:e2e`'s absence hides a *red suite*, not just an unrun one, and "zero skips attributable to missing infrastructure" is not reachable by adding service containers. Repairing nine legacy-engine tests against a moved schema is its own slice. Original text follows.
...
4. **Alloc ceilings for the durable path.** Extend `workflowAllocCeilings` (`benchmark_util.go:328-335`) — or add a sibling map so the legacy and durable sets stay distinguishable — with `MaxAllocsPerOp` for the new benchmarks, calibrated from at least three CI artifacts across different `cpu:` headers, as `-suggest` (`cmd/bench-check/suggest.go`) is built to do. Leave `MaxNsPerOp` and `MinThroughput` **unset** for the durable set, and ~~make `Check` (`:538-602`) treat an unset threshold as "not gated" rather than zero~~ — **corrected by S5-A, 2026-08-09: `Check` already does this**, at `:569`, `:581`, and `:591`, each guarded by a `value, ok := map[name]; ok` lookup. No change is needed and none should be made. **The real hazard the task description hides is one scope up**: `Check`'s `required` set (`:547-556`) unions the keys of *all three* maps and then fails any name with no result in the suite (`:558-566`). So adding durable benchmark names to `workflowAllocCeilings` — which `ResolveWorkflowThresholds` (`:481-500`) copies into every profile's thresholds unconditionally — would make the existing `test:benchmark` job fail with `required benchmark result missing`, because its package list (`.gitlab-ci.yml:1250`) does not include `internal/integration`. **That, not the zero-value question, is why the durable set needs a sibling map plus a way for `bench-check` to select which set applies to a given run.** Extending `workflowAllocCeilings` in place is not an option; the "or" in this task's first sentence is a false choice.
